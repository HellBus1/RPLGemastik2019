...
print(os.getcwd())
base_path = 'A:\\proyekGemastik\\Repository\\tensorflow\\'
with open(base_path + 'conf\\conf.vgg16.json') as f:
    config = json.load(f)
...
if model_name == "vgg16":
    ...
    #    lastLayer = base_model.layers[-0]#[-3]
...
train_labels = os.listdir(base_path + train_path)
...
for i, label in enumerate(train_labels):
    cur_path = base_path + train_path + "\\" + label
    count = 1
...
output = open(base_path + model_name + '\\encoder.pickle', 'wb')
...
h5f_data = h5py.File(base_path + features_path, 'w')
h5f_data.create_dataset('dataset_1', data=np.array(features))

h5f_label = h5py.File(base_path + labels_path, 'w')
...
with open(base_path + model_path + str(test_size) + ".json", "w") as json_file:
    json_file.write(model_json)

# save weights
model.save_weights(base_path + model_path + str(test_size) + ".h5")
